...
async function importLnaQuestions() {
  console.log('Starting LNA rapid diagnostic question import...');
  ...
  console.log(`Found ${rows.length} LNA questions in CSV. Importing...`);
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      if (result.rowCount === 0) {
        ...
        console.log(`Row ${rowNum} (${row['Question_ID']}): imported`);
        imported++;
      }
    } catch (err) {
